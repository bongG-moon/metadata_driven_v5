...
- 최대 4회 연계 Agent Orchestrator Router: `import_ready_flows/08_agent_orchestrator_router_flow_v5_standalone.json`
- 최대 4단계 문서 기반 순차 Workflow Router: `import_ready_flows/09_workflow_orchestrator_flow_v5_standalone.json`
- Workflow Skill 등록·검수·저장 Flow: `import_ready_flows/10_workflow_skill_saving_flow_v5_standalone.json`
- 06 커스텀 원본: `langflow_components/route_flow/`
- 07 커스텀 원본: `langflow_components/route_flow_v2/`
- 08 커스텀 원본: `langflow_components/route_flow_v3/`
- 09 커스텀 원본: `langflow_components/route_flow_v4/`
- 10 커스텀 원본과 입력 예시: `langflow_components/workflow_skill_saving_flow/`
- Workflow Skill 복사·붙여넣기 입력 예시: `langflow_components/workflow_skill_saving_flow/INPUT_EXAMPLES.md`
- Route V3 구현·운영 계약: `docs/ROUTE_V3_ORCHESTRATOR_IMPLEMENTATION.md`
- Route V4 구현·운영 계약: `docs/ROUTE_V4_WORKFLOW_ORCHESTRATOR_IMPLEMENTATION.md`
...
- v5는 별도 데이터 복제 없이 v4와 같은 `datagov` 데이터베이스의 `agent_v4_domain_items`, `agent_v4_table_catalog_items`, `agent_v4_main_flow_filters`, `agent_v4_result_store`, `agent_v4_session_states` collection을 직접 재사용합니다. Route V4의 반복 업무 정의는 같은 데이터베이스의 `agent_v4_workflow_skills`에 별도로 저장합니다.
...
23. Tool이 없는 의도 분석·pandas 코드·답변·Metadata QA·저장 후보 추출 7개 단계는 Langflow 기본 `Language Model`로 통일했습니다. 실제 Cached Run Flow Tool이 연결된 Route V2와 V3만 기본 `Agent`를 유지하므로, 일반 모델 호출에는 빈 `tools=[]`가 전달되지 않습니다.
24. Route V3는 `return_direct=false`, 기본 Agent `max_iterations=5`, 시스템 프롬프트 최대 Tool 호출 4회로 구성합니다. Tool은 고정 `question`과 선택 `upstream_result_ref`만 받고, 하위 결과를 `route_v3.tool_result.v1` 축약 계약으로 반환한 뒤 마지막 Agent 답변만 한 번 저장합니다.
25. Data Analysis는 명시적 `upstream_result_ref`가 있을 때만 같은 세션의 MongoDB 결과 전체를 `upstream_result` alias로 복원합니다. 다음 조회 파라미터는 Table Catalog의 신뢰된 `source_config.upstream_bindings`로만 연결하며, 선언 누락·충돌·상한 초과 시 broad query를 실행하지 않습니다. 일반 단일 실행과 기존 후속 질문 경로는 이 모드를 사용하지 않습니다.
26. Route V4는 자연어 번호 목록 또는 화면에 등록한 Workflow JSON을 `workflow.plan.v1`로 먼저 확정하고, Langflow 기본 Loop가 최대 4단계를 한 번에 하나씩 실행합니다. 계획 생성과 마지막 종합에만 기본 Language Model을 사용하며, 단계 실행 중에는 지정 Tool 하나만 호출합니다. `depends_on`은 순서를, `handoff=result_ref`는 실제 MongoDB 결과 전달을 담당합니다.
27. Route V4는 운영 기본값으로 `datagov.agent_v4_workflow_skills`의 active 문서를 조회하고, 현재 질문과 관련된 후보만 최대 8개·64KB로 제한해 계획 모델과 파서에 함께 전달합니다. `inline_seed`는 명시적 테스트 모드이며 MongoDB 오류 시 자동 fallback하지 않습니다.
28. Workflow Skill 저장 Flow는 자연어 등록 요청에서 LLM 후보를 한 번 생성한 뒤 Python이 최대 4단계, Tool 이름, dependency 순서, `result_ref` producer/consumer, 32KB payload를 결정론적으로 검증합니다. 기본은 `dry_run=true`이며 `replace`는 유사 1건 교체, 0건 신규 저장, 복수 건 차단으로 동작합니다.
...
- 전체 pytest 290개와 대표 dummy 질문 23/23이 통과했습니다.
- Langflow Desktop 설치본(`langflow 1.8.2`, `lfx 0.3.4`)에서 현재 10개 Flow의 node template 153/153을 파싱했습니다. 이전 8개 Flow는 별도 임시 config/SQLite 격리 서버에서 개별 JSON 8/8과 당시 통합 `00` JSON을 HTTP 201로 확인했으며, Route V4와 Workflow Skill 저장 Flow는 최신 export/import JSON의 node·edge·source 동기화 계약을 검증했습니다.
- Route V3의 연계 호출은 `agent_v4_result_store`를 사용하므로 `MONGO_URL`과 같은 부모/자식 `session_id`가 필수입니다. 저장된 결과가 없거나 다른 세션의 ref이면 후속 조회를 fail-closed로 중단합니다.
...
- `langflow_components`의 Python 83개에는 역할·입력·출력·처리 흐름·유지보수 포인트와 전체 함수 1361/1361의 인접 한글 설명이 들어 있습니다. private helper, 클래스 메서드, async 함수와 중첩 함수도 포함합니다.
