"""MongoDB result-store reader used by the standalone API server.

This module intentionally has no imports from the repository outside the
API_SERVER folder, so the folder can be copied to a deployment host as-is.
"""
...
def load_data_ref_rows(
    ...
    if not isinstance(data_ref, dict):
        raise ValueError("data_ref must be an object.")
...
def rows_from_data_ref_document(
    ...
    if not rows:
        ...
        rows = _row_list(document_data.get("rows")) if isinstance(document_data, dict) else []
    if not rows:
        document_result = document.get("result")
        rows = _row_list(document_result.get("rows")) if isinstance(document_result, dict) else []
    if not rows:
    ...
    row_count = _positive_int(
        selected_dict.get("row_count"),
        _positive_int(document.get("row_count"), len(rows)),
    )
    columns = (
        _string_list(selected_dict.get("columns"))
