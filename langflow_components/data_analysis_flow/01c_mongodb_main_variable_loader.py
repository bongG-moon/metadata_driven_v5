...
def load_main_variable_metadata(
    ...
    if not config["mongo_uri"]:
        return _result("skipped", [], config, status_filter, [{"type": "missing_mongo_uri", "message": "MONGODB_URI가 없어 메인 변수 메타데이터를 불러오지 않았습니다."}])
...
class MongoDBMainVariableLoader(Component):
    ...
    inputs = [
        ...
        MessageTextInput(name="collection_name", display_name="메인 변수 컬렉션", required=False, value=DEFAULT_COLLECTION, advanced=True),
