...
def load_table_catalog_metadata(
    ...
    if not config["mongo_uri"]:
        return _result("skipped", [], config, status_filter, [{"type": "missing_mongo_uri", "message": "MONGODB_URI가 없어 테이블 카탈로그 메타데이터를 불러오지 않았습니다."}])
...
class MongoDBTableCatalogLoader(Component):
    ...
    inputs = [
        ...
        MessageTextInput(name="collection_name", display_name="테이블 카탈로그 컬렉션", required=False, value=DEFAULT_COLLECTION, advanced=True),
