...
def _enforce_selected_domain_filter_contracts(
    ...
    The LLM may emit ``eq`` without a value for a metadata-owned condition such
    as ``not_blank``.  An empty value must never reach the Fast resolver.  The
    selected Domain is authoritative only when the LLM condition is incomplete;
    a valid, more-specific user condition is preserved.
    """
