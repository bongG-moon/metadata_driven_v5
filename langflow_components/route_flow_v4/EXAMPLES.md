...
## 2. 이상 LOT 조회 후 HOLD 이력 분석

사용자 질문 예시:

```text
오늘 이상 LOT을 분석하고 해당 LOT의 HOLD 이력과 주요 원인을 알려줘.
```
  ...
  "steps": [
    {
      ...
      "question": "{{user_question}}에서 요청한 일자와 기준으로 이상 LOT을 분석해줘.",
      "depends_on": [],
      "handoff": "none",
      "on_error": "stop"
    },
...
- `workflow_key` 입력: `anomaly_lot_hold_history`
- Inline Workflow 정의: `workflow_key: anomaly_lot_hold_history`
- Inline Workflow 정의: `anomaly_lot_hold_history`
