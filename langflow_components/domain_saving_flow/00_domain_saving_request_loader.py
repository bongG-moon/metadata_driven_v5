# -*- coding: utf-8 -*-
# =============================================================================
# 컴포넌트 개요: 00 도메인 등록 요청 로더
# 역할: 자연어 도메인 메타데이터 등록 요청을 시작합니다. 기본값은 테스트 실행입니다.
# 주요 입력: 원문 텍스트 (raw_text) · 필수, 중복 처리 방식 (duplicate_action), 테스트 실행 (dry_run)
# 주요 출력: 페이로드 출력 (payload_out)
# 처리 흐름: 자연어 도메인 등록 요청을 duplicate action과 기본 테스트 실행(dry_run)이 포함된 안전한 표준 페이로드로 초기화합니다.
...
class DomainSavingRequestLoader(Component):
    ...
    description = "자연어 도메인 메타데이터 등록 요청을 시작합니다. 기본값은 테스트 실행입니다."
    inputs = [
        ...
        BoolInput(name="dry_run", display_name="테스트 실행 (저장하지 않음)", value=True),
    ]
