# -*- coding: utf-8 -*-
# =============================================================================
# 컴포넌트 개요: 01C 메타데이터 QA 메인 필터 로더
# 역할: MongoDB에서 메인 플로우 필터 메타데이터를 읽기 전용으로 불러옵니다.
# 주요 입력: MongoDB 연결 URI (mongo_uri), MongoDB 데이터베이스 (mongo_database), 메인 필터 컬렉션 (collection_name), 조회 제한 (limit), 상태
#        필터 (status_filter)
# 주요 출력: 메인 필터 (main_flow_filters)
# 처리 흐름: 표준 필터 이름·별칭·연산자·값 형식을 설명할 활성 메인 필터 문서를 읽습니다.
...
class MetadataQaMainFilterLoader(Component):
    ...
    # Langflow 출력 함수: '메인 필터 (main_flow_filters)' 포트가 요청될 때 실행됩니다.
