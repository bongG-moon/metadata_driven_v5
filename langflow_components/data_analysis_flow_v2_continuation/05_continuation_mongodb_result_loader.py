...
def load_previous_result(payload_value: Any, mongo_uri: str = "", mongo_database: str = "", collection_name: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        if explicit_orchestration and _continuation_requested(next_payload):
            expiry_error = _continuation_expiry_error(doc)
            if expiry_error:
...
def _restore_explicit_upstream_result(
    ...
    manifest = stored_payload.get("storage_manifest") if isinstance(stored_payload.get("storage_manifest"), dict) else {}
    result_manifest = manifest.get("result_rows") if isinstance(manifest.get("result_rows"), dict) else {}
    continuation_requested = _continuation_requested(payload)
    if (
        (continuation_requested and result_manifest.get("complete") is not True)
        or (not continuation_requested and result_manifest.get("complete") is False)
    ):
        return _mark_error(
            payload,
            database,
    ...
    continuation_error = _continuation_resume_error(payload, stored_payload)
    if continuation_error:
        return _mark_error(payload, database, collection_name, ref_id, [continuation_error])
...
def _continuation_resume_error(
    payload: dict[str, Any],
    stored_payload: dict[str, Any],
) -> dict[str, Any] | None:
    """Bind a resume request to the exact complete stage-1 plan stored by MongoDB."""

    request = payload.get("request") if isinstance(payload.get("request"), dict) else {}
    continuation = request.get("continuation") if isinstance(request.get("continuation"), dict) else {}
    if not continuation:
        return None
    contract = continuation.get("continuation_contract") if isinstance(continuation.get("continuation_contract"), dict) else {}
    current_plan = payload.get("intent_plan") if isinstance(payload.get("intent_plan"), dict) else {}
    stored_plan = stored_payload.get("intent_plan") if isinstance(stored_payload.get("intent_plan"), dict) else {}
    current_dependent = current_plan.get("dependent_retrieval_plan") if isinstance(current_plan.get("dependent_retrieval_plan"), dict) else {}
    stored_dependent = stored_plan.get("dependent_retrieval_plan") if isinstance(stored_plan.get("dependent_retrieval_plan"), dict) else {}
    if not contract or not current_dependent or not stored_dependent:
        return {
            "type": "continuation_contract_missing",
            "message": "저장 결과와 현재 요청에서 continuation 계약을 모두 확인할 수 없습니다.",
        }

    request_session = str(request.get("session_id") or "").strip()
    if not request_session or str(contract.get("session_id") or "").strip() != request_session:
        return {
            "type": "continuation_session_mismatch",
            "message": "continuation 계약의 session_id가 현재 요청과 일치하지 않습니다.",
        }

    for key in ("version", "plan_id", "plan_hash"):
        expected = str(contract.get(key) or "").strip()
        if not expected or expected != str(stored_dependent.get(key) or "").strip() or expected != str(current_dependent.get(key) or "").strip():
            return {
                "type": "continuation_contract_mismatch",
                "message": f"저장 결과와 continuation 계약의 {key}가 일치하지 않습니다.",
            }
    if int(contract.get("max_stages") or 0) != 2:
        return {
            "type": "continuation_stage_limit_invalid",
            "message": "continuation max_stages는 2여야 합니다.",
        }
    try:
        current_stage_index = int(contract.get("current_stage_index"))
        next_stage_index = int(contract.get("next_stage_index"))
    except Exception:
        return {
            "type": "continuation_stage_index_invalid",
            "message": "continuation 단계 번호가 유효하지 않습니다.",
        }
    stored_runtime = stored_dependent.get("runtime") if isinstance(stored_dependent.get("runtime"), dict) else {}
    current_runtime = current_dependent.get("runtime") if isinstance(current_dependent.get("runtime"), dict) else {}
    if (
        current_stage_index != 0
        or next_stage_index != 1
        or int(stored_runtime.get("active_stage_index", -1)) != current_stage_index
        or str(stored_runtime.get("status") or "") != "pending"
        or int(current_runtime.get("active_stage_index", -1)) != next_stage_index
    ):
        return {
            "type": "continuation_stage_mismatch",
            "message": "저장된 단계와 재개하려는 단계가 연속되지 않습니다.",
        }
    expected_ref = f"continuation:{contract.get('plan_id')}:{contract.get('plan_hash')}"
    if str(continuation.get("continuation_ref") or "").strip() != expected_ref:
        return {
            "type": "continuation_ref_mismatch",
            "message": "continuation_ref가 저장 계약과 일치하지 않습니다.",
        }
    stored_analysis = stored_payload.get("analysis") if isinstance(stored_payload.get("analysis"), dict) else {}
    if str(stored_analysis.get("status") or "").strip().lower() not in {"ok", "success"}:
        return {
            "type": "continuation_upstream_not_complete",
            "message": "1차 분석이 정상 완료되지 않아 continuation을 재개할 수 없습니다.",
        }
    manifest = stored_payload.get("storage_manifest") if isinstance(stored_payload.get("storage_manifest"), dict) else {}
    result_manifest = manifest.get("result_rows") if isinstance(manifest.get("result_rows"), dict) else {}
    if result_manifest.get("complete") is not True:
        return {
            "type": "continuation_upstream_incomplete",
            "message": "1차 결과 행의 완전성이 확인되지 않아 continuation을 재개할 수 없습니다.",
        }
    stages = [item for item in stored_dependent.get("stages", []) if isinstance(item, dict)]
    if len(stages) != 2 or _canonical_json(contract.get("input_bindings", [])) != _canonical_json(
        stages[next_stage_index].get("input_bindings", [])
    ):
        return {
            "type": "continuation_binding_mismatch",
            "message": "continuation input_bindings가 저장된 다음 단계와 일치하지 않습니다.",
        }
    return None


# 함수 설명: 현재 요청이 공개 continuation 계약을 사용한 명시적 재개인지 확인합니다.
def _continuation_requested(payload: dict[str, Any]) -> bool:
    request = payload.get("request") if isinstance(payload.get("request"), dict) else {}
    continuation = request.get("continuation")
    return isinstance(continuation, dict) and bool(continuation)


# 함수 설명: MongoDB TTL 삭제 지연과 무관하게 저장 결과의 UTC 만료 시각을 즉시 검증합니다.
def _continuation_expiry_error(document: dict[str, Any]) -> dict[str, Any] | None:
    raw = document.get("expires_at")
    if raw in (None, ""):
        return {
            "type": "continuation_expiry_missing",
            "message": "저장 결과의 expires_at을 확인할 수 없어 continuation을 재개하지 않았습니다.",
        }
    try:
        ...
        return {
            "type": "continuation_expiry_invalid",
            "message": "저장 결과의 expires_at 형식이 유효하지 않아 continuation을 재개하지 않았습니다.",
        }
    if expires_at <= datetime.now(timezone.utc):
        return {
            "type": "continuation_result_expired",
            "message": "만료된 저장 결과로 continuation을 재개할 수 없습니다.",
        }
    return None


# 함수 설명: 공개 계약과 저장 계약의 list/dict를 키 순서와 무관한 문자열로 비교합니다.
...
# 함수 설명: 같은 source_alias의 복원 결과를 교체하고 없으면 기존 순서를 유지해 추가합니다.
