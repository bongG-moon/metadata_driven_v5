...
def _intent_section(payload: dict[str, Any]) -> str:
    ...
    route_resolution = plan.get("route_resolution") if isinstance(plan.get("route_resolution"), dict) else {}
    analysis = payload.get("analysis") if isinstance(payload.get("analysis"), dict) else {}
    final_route = str(route_resolution.get("final_route") or analysis.get("execution_route") or "").strip()
    lines = ["### 의도 분석"]
    for label, value in (
        ("분석 유형", plan.get("analysis_kind") or intent_trace.get("analysis_kind")),
        ("예상 실행 경로", _route_display_label(route_resolution.get("intent_candidate"))),
        ("최종 실행 경로", _route_display_label(final_route)),
        ("Fast 레시피", route_resolution.get("final_recipe") or route_resolution.get("candidate_recipe")),
        ("경로 결정 근거", route_resolution.get("final_reason_codes") or route_resolution.get("candidate_reason_codes")),
...
# 함수 설명: `_route_display_label()`은 내부 route 값을 사용자가 구분하기 쉬운 Fast/Complex/Blocked 한글 표기로 바꿉니다.
...
def _pandas_section(payload: dict[str, Any]) -> str:
    ...
    execution_route = str(analysis.get("execution_route") or "").strip().lower()
    deterministic_function = pandas_trace.get("deterministic_function") if isinstance(pandas_trace.get("deterministic_function"), dict) else {}
    lines = ["### pandas 코드/실행"]
    for label, value in (
        ("상태", pandas_trace.get("status") or analysis.get("status")),
        ("실행 경로", _route_display_label(execution_route)),
        ("코드 생성 방식", pandas_trace.get("code_generation_type") or analysis.get("code_generation_type")),
        ("Fast 레시피", analysis.get("fast_path_recipe") or deterministic_function.get("recipe")),
        ("고정 실행 함수", deterministic_function),
    ...
    deterministic_logic_code = str(pandas_trace.get("deterministic_logic_code") or "").strip()
    code = (
        deterministic_logic_code
        if execution_route == "fast" and deterministic_logic_code
        else effective_code or str(pandas_trace.get("generated_code") or analysis.get("analysis_code") or "").strip()
    )
