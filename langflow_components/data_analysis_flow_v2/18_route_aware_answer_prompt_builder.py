...
V2_ANSWER_PROMPT_REFERENCE_REWRITES = {
...
# 함수 설명: V2 Complex 답변에서 각 정보가 한 영역에만 존재하도록 context 변수를 구성합니다.
def build_v2_variables(payload_value: Any) -> dict[str, Any]:
    """Build the V2 Complex answer context with one owner per repeated field."""

    payload = _payload(payload_value)
    applied_scope = _compact_applied_scope(payload)
    criteria = _applied_criteria(payload)
    if criteria:
        applied_scope["criteria"] = criteria
    pandas_execution = applied_scope.get("pandas_execution")
    if isinstance(pandas_execution, dict):
        pandas_execution.pop("row_count", None)
        pandas_execution.pop("columns", None)

    answer_context = _answer_context(payload)
    answer_context.pop("result_shape", None)
    answer_context.pop("applied_criteria", None)
    return {
        "question": payload.get("request", {}).get("question", ""),
        "result_summary_json": _json_dumps(payload.get("data", {})),
        "applied_scope_json": _json_dumps(applied_scope),
        "answer_context_json": _json_dumps(answer_context),
        "warnings_errors_json": _json_dumps(
            {
                "warnings": payload.get("trace", {}).get("warnings", []),
                "errors": payload.get("trace", {}).get("errors", []),
            }
        ),
    }


# 함수 설명: V2 Complex 경로에서만 중복 제거된 답변 프롬프트를 생성합니다.
def build_route_aware_answer_prompt(
    payload_value: Any,
    prompt_template: Any,
    domain_answer_guidance: Any = "",
) -> str:
    """Render the answer prompt only for a successfully selected Complex route."""

    payload = _payload(payload_value)
    contract = payload.get("simple_analysis_contract") if isinstance(payload.get("simple_analysis_contract"), dict) else {}
    analysis = payload.get("analysis") if isinstance(payload.get("analysis"), dict) else {}
    route = str(analysis.get("execution_route") or contract.get("route") or "complex").strip().lower()
    if route != "complex":
        return ""
    variables = build_v2_variables(payload)
    variables["domain_answer_guidance"] = _text(domain_answer_guidance)
    return _render_answer_prompt_template(prompt_template, variables)


# 함수 설명: 이전 템플릿 참조를 단일 소유 영역으로 치환한 뒤 답변 프롬프트를 조립합니다.
def _render_answer_prompt_template(template_value: Any, variables: dict[str, Any]) -> str:
    template = _text(template_value)
    if not template:
        return ""
    for previous, current in V2_ANSWER_PROMPT_REFERENCE_REWRITES.items():
...
# 함수 설명: Langflow Message 또는 일반 값을 답변 프롬프트용 문자열로 정규화합니다.
