...
- 원문에 metric의 합산 가능 여부나 기본 집계 방식이 있으면 `metric_semantics`도 보존한다. `filter_mappings`에 물리 컬럼과 canonical 실행 컬럼의 대응이 있으면 `metric_semantics`의 key는 **canonical 실행 컬럼만** 사용한다. 대응이 없을 때만 실제 조회 컬럼을 key로 사용한다. 같은 필드에 물리 컬럼 key와 canonical key를 함께 만들지 않으며, `HOLD_LOT_COUNT`처럼 결과에 붙일 이름을 key로 만들지 않는다.
...
- `value_transform`은 선택 필드다. 원문에 숫자 변환·단위 환산·배수 근거가 없으면 key 자체를 반환하지 않는다. 빈 값, `null`, placeholder multiplier를 넣지 않는다.
...
- `filter_mappings`는 조회 후 pandas filter·집계에서 쓰는 결과 컬럼 계약이다. 왼쪽은 canonical 실행 key, 오른쪽은 최종 조회 결과에 실제 존재하는 source column이다.
- `required_param_mappings`는 SQL/API 조회 입력 계약이다. SQL의 nested WHERE·subquery·placeholder에만 쓰이는 물리 컬럼은 최종 SELECT 결과에 없어도 여기에 유지할 수 있으며, 그런 컬럼을 `filter_mappings`에 중복 등록하지 않는다.
- `columns`에는 DB 원본 테이블명이 아니라 query/API/문서 조회가 반환하는 **최상위 SELECT의 최종 컬럼명**을 기록한다. SQL `AS`가 있으면 alias 이후 이름을 사용한다. `SELECT *`처럼 최종 컬럼을 확정할 수 없는 SQL은 원문에서 확인 가능한 columns만 기록하고 추측하지 않는다.
- metric도 물리명이 canonical key와 다르면 `filter_mappings`에 함께 선언한다. `metric_semantics`와 `default_detail_columns`는 canonical key를 사용하며, 물리 컬럼명은 `columns`, `query_template`, mapping의 오른쪽에만 한 번 보존한다.
