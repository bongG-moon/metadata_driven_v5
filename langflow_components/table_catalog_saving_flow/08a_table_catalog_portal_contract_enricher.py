# -*- coding: utf-8 -*-
# =============================================================================
# 컴포넌트 개요: 08A 테이블 카탈로그 Portal 계약 보강기
# 역할: 기존 03 Writer/Response 결과를 바꾸지 않고 Portal이 표시하는 등록 과정 필드를
#       additive 방식으로만 채웁니다.
...
PORTAL_RESPONSE_MODE = "legacy_table_catalog_writer_with_initial_transform_v1"
...
def enrich_portal_contract(response_value: Any, authoring_payload_value: Any) -> dict[str, Any]:
    ...
    The lightweight Table Catalog Flow deliberately omits the rev_2 snapshot and
    contract guard chain.  The Portal, however, still renders a stable API shape
    containing ``metadata_authoring``, ``data``, ``write_result`` and ``trace``.
    This adapter fills only missing display fields after the legacy Writer has
    reached its decision.  It must not turn a result into ``needs_input`` or
    rewrite the Writer's status/message/rows.
    """
    ...
    # Existing legacy summary fields are authoritative.  The Portal-specific
    # additions below use setdefault so this component cannot overwrite a
    # future response builder that supplies richer values itself.
    metadata_authoring.setdefault("contract_version", PORTAL_CONTRACT_VERSION)
    metadata_authoring.setdefault("metadata_type", "table_catalog")
    metadata_authoring.setdefault("metadata_label", "테이블 카탈로그")
    ...
    metadata_authoring["contract_validation"] = {
        **validation,
        **existing_validation,
    }
    response["metadata_authoring"] = metadata_authoring

    # Do not alter Writer errors or the response status.  A Portal caller can
    # still use trace to distinguish this compatible lightweight route.
...
def _portal_validation(
    ...
    warnings = _unique_diagnostics(
        [
            *_list(authoring.get("warnings")),
            *_list(review.get("warnings")),
        ]
    )
    ...
    if response_status in {"saved", "dry_run", "skipped"} and not errors:
        ...
        # The legacy Writer is the only checker on this route.  A completed
        # response without an explicit pass/fail remains reviewable, never
        # becomes a new blocking decision here.
        status = "reviewed"
    return {
        "status": status,
        "mode": "legacy_writer",
...
def _keys(write_result: dict[str, Any], items: list[Any]) -> list[str]:
    ...
    return [
        str(_dict(value).get("dataset_key") or _dict(value).get("key") or "").strip()
        for value in items
        if str(_dict(value).get("dataset_key") or _dict(value).get("key") or "").strip()
    ]
...
class TableCatalogPortalContractEnricher(Component):
    display_name = "08A 테이블 카탈로그 Portal 계약 보강기"
