...
function renderAccessControls() {
  ...
  const viewer = state.portal.viewer;
  $("#viewer-name").textContent = viewer.name;
  $("#viewer-role").textContent = `${viewer.role} · ${viewer.employee_id}`;

  const profileImage = $("#my_face");
  if (profileImage) {
    const employeeId = String(viewer.employee_id || "2069026").trim();
    const fallbackImageUrl = `http://skynet.skhynix.com/portalWeb/uploadfile/pictures/${encodeURIComponent(employeeId)}.jpg`;
    profileImage.src = viewer.profile_image_url || fallbackImageUrl;
    profileImage.alt = `${viewer.name || "사용자"} 프로필 사진`;
  }

  const mobileUser = $(".mobile-user");
  if (mobileUser) {
    mobileUser.textContent = String(viewer.name || "사용자").slice(0, 1);
  }
}
