...
`mail_renderer.py`가 원본 답변의 Markdown 제목, 목록, 강조, 코드 블록, 파이프 표와 HTTP(S) 링크를 메일 HTML로 변환합니다. 보고서 링크는 본문에 유지하고 하단에도 버튼 형태로 제공합니다. Outlook용 표 레이아웃과 인라인 스타일을 사용하며 외부 이미지, 스크립트, 추가 패키지는 필요 없습니다. HTML 원문은 안전하게 문자로 표시합니다.
