...
This self-contained process is deployed separately from the interactive CUBE
callback server. It polls the Portal's ``portal_schedules`` collection,
...
# These local files contain the exact GAIA/CUBE request contract and Markdown
# renderer needed for a standalone deployment. No callback-server import is
# required at runtime.
from cube_runtime import (
    ExternalApiError,
    GaiaRequestError,
    GaiaResponseError,
    Settings,
...
class SchedulerWorker:
    ...
    async def run_once(
        ...
        *,
        ...
        for candidate in candidates:
            # Shutdown is cooperative: finish an in-flight GAIA/CUBE request,
            # but do not claim another schedule after the service is asked to
            # stop.  A claim that was already acquired remains auditable and
            # is finalized by ``execute_claimed_schedule``.
                ...
                if not self._claim_allows_delivery(claimed):
                    ...
                    try:
                        await send_cube_message(
                            ...
                            build_scheduled_result_message(
                                question,
                                gaia_fallback_message(self.gaia_settings, exc),
                            ),
                        )
    ...
    async def run_forever(self, *, stop_event: asyncio.Event | None = None) -> None:
        """Keep polling until an optional application shutdown event is set.

        The CLI intentionally calls this without an event and retains its
        previous infinite-loop behavior.  The HCP FastAPI wrapper provides an
        event so Uvicorn shutdown does not wait for a full poll interval.
        """
        ...
        async with httpx.AsyncClient() as client:
            while stop_event is None or not stop_event.is_set():
                try:
                    executed = await self.run_once(client, stop_event=stop_event)
                    if executed:
                ...
                try:
                    await asyncio.wait_for(
                        stop_event.wait(),
                        timeout=self.scheduler_settings.poll_seconds,
                    )
                except TimeoutError:
                    # Normal polling timeout.  The outer loop starts the next
                    # due-schedule scan unless shutdown has been requested.
                    continue
