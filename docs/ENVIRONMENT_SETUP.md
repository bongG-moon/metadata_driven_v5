...
운영 기본 API Router에는 Run Flow 노드가 없습니다. 별도로 제공되는 `06_agent_tool_router_flow_v5_standalone.json`에는 선택한 Flow ID를 우선 사용하는 Cached Run Flow Tool 6개가 있으며, 실시간 생산 Report는 `07_realtime_production_report_flow_v5_standalone.json`으로 분리되어 있습니다. 모두 `Cache Flow=true`입니다. 06의 여섯 번째 Tool은 질문에 `분석`이 있고 `실시간 생산 분석`·`실시간 분석`·`실시간 생산분석` 중 하나가 있을 때만 07번 Flow를 실행하며, Agent 오선택 시에도 실행 직전 Gate가 차단합니다. 06의 `대상 Flow`는 기본 Run Flow와 동일하게 현재 프로젝트의 Flow만 표시합니다. `Refresh List`를 누르면 저장된 Flow ID를 기준으로 변경된 이름과 수정 시각을 자동 반영하며, import 직후 ID가 비어 있을 때는 프로젝트 내 유일하게 일치하는 이름의 ID를 자동 저장합니다. 일치하는 이름이 없거나 중복이면 해당 Tool에서 대상을 한 번 선택해야 합니다. 이 설정은 하위 Flow 그래프만 캐시하고 데이터 조회·pandas·LLM 결과는 캐시하지 않습니다.
...
07 Realtime Production Report Flow는 예시 실행에서 내부 더미 Snapshot 500행을 사용하고 `HTML Report API 주소=http://127.0.0.1:8765`, 링크 TTL 4시간을 화면 입력으로 노출합니다. 운영에서는 더미 생성 노드를 실제 판정 Snapshot 로더로 교체하되 Report Builder의 `production.judgement.dataset.v1` 입력을 유지합니다.

??? Workflow Skill 저장 Flow는 같은 `MONGO_URL`, `datagov`, `agent_v4_workflow_skills`를 기존 항목 로더·유사 항목 조회기·Writer에 입력합니다. `dry_run=true`가 기본이며 실제 저장 때만 끕니다. 저장 후 ??? Workflow Orchestrator는 다음 요청에서 MongoDB를 다시 조회하므로 Flow JSON을 재export할 필요가 없습니다.
