# Route V4 업무 Workflow 문서 작성 가이드

이 폴더는 Route V4가 실행할 반복 업무를 사람이 검토할 수 있는 문서와 seed registry JSON으로 함께 관리하는 예시다. 문서 파일을 런타임에서 직접 읽지는 않는다. 운영 Workflow는 `10 Workflow Skill 저장 Flow`로 `datagov.agent_v4_workflow_skills`에 저장하고, Route V4의 `00A Workflow Registry 로더`가 다음 실행부터 질문 관련 후보를 조회한다.
...
├─ wb_daily_production_metadata.md
└─ anomaly_lot_hold_history.md
```
...
| `tool_name` | 예 | Route V4에 연결된 정확한 Tool 이름 |
...
`depends_on`과 `handoff`는 서로 다른 개념이다. 예를 들어 “생산량을 조회한 뒤 WB 정의를 조회”는 실행 순서만 필요하므로 `depends_on=["production"]`, `handoff="none"`이다. 반대로 “이상 LOT을 조회한 뒤 그 LOT의 HOLD 이력 조회”는 앞 단계의 실제 LOT 목록이 필요하므로 `handoff="result_ref"`를 사용한다.

## 자연어 업무 입력 예시

V4 Playground에는 다음처럼 사람이 작성한 업무 절차를 그대로 입력할 수 있다. 첫 Language Model이 이 내용을 `workflow.plan.v1` JSON으로 변환하고, 검증 파서가 Tool 이름·의존 순서·최대 4단계를 확인한 뒤에만 Loop를 시작한다.
...
4. `10 Workflow Skill 저장 Flow`에서 `dry_run=true`로 자연어 등록 입력을 실행한다.
5. 생성된 Tool·단계·dependency·handoff를 확인한 뒤 `dry_run=false`로 다시 실행한다.
6. Route V4의 `registry_source=mongodb`, `MONGO_URL`, database/collection 입력이 저장 Flow와 같은지 확인한다.
...
- 4단계를 초과하지 않았는가
- 모든 `tool_name`이 Route V4에 실제 연결되어 있는가
- `depends_on`은 항상 앞 단계만 가리키는가
- `result_ref`가 필요한 Tool이 해당 입력을 지원하는가
- 저장 Tool은 사용자 요청에 저장 의도가 명시된 경우에만 포함했는가
