...
## 8. Workflow 하위 Tool Handoff

Workflow Orchestrator 하위 Tool의 공개 입력은 node ID가 없는 고정 계약입니다.
...
- 하위 Tool 호출은 검증된 Workflow 계획에 따라 요청당 최대 4회 순차 실행합니다.
- 08의 허용 Tool은 기존 조회·저장 5종과 `run_visualization`을 합친 6종입니다. 계획 Prompt는 이름뿐 아니라 설명과 ref producer/consumer capability catalog도 함께 사용합니다.
- 단계 observation은 하위 호환 계약인 `route_v3.tool_result.v1`의 `status`, `summary`, `result_ref`, `result_ref_meta`, `entity_ids`, `handoff_usable`, 제한된 오류만 포함합니다.
- 전체 rows, source payload, SQL, trace, intent plan, pandas 코드는 단계 observation에 넣지 않습니다.
- `status=error` 또는 `handoff_usable=false`인 결과에 의존하는 후속 호출은 중단합니다.

Data Analysis에서 명시적 ref를 받으면 같은 `session_id`의 `datagov.agent_v4_result_store` 문서만 읽고, 저장된 전체 `payload.result_rows`를 `runtime_sources.upstream_result`로 복원합니다. `run_visualization`도 같은 저장소에서 ref를 복원해 HTML 차트를 만들며 새 result_ref는 생성하지 않습니다. 다음 데이터 조회의 파라미터는 Table Catalog의 `source_config.upstream_bindings` 선언으로만 결정합니다.
...
```json
{
  ...
  "user_question": "오늘 DA 생산량과 재공, 데이터 소스를 알려줘",
  "max_steps": 4,
  "steps": [
    {
      "step_index": 1,
...
- `tool_name`은 08 Workflow Orchestrator 화면의 허용 Tool 목록과 정확히 일치해야 합니다.
...
부모 08 Workflow Orchestrator만 질문과 최종 답변을 저장합니다. child Chat Input/Output은 이름 기반 Tool의 runtime tweak로 메시지 저장을 끕니다.
...
```json
{
  ...
  "payload": {
    ...
    "description": "DA 생산량과 재공 조회 후 사용 데이터셋 정의를 확인합니다.",
    "aliases": [],
    "intent_examples": [],
    "keywords": [],
    "excluded_keywords": [],
...
- 저장 전 최대 4단계, 허용 Tool 6종, 앞 단계 dependency, `result_ref` producer/consumer, 단계 질문 4,000자, 전체 payload 32KB를 Python으로 다시 검증합니다.
