...
- `langflow_components` Python: 68개 전체
- 활성 Custom Component 원본: 67개
...
공개 업무 함수와 Langflow 출력 메서드에는 각각 `주요 함수`, `Langflow 출력 함수` 설명을 둔다. `_payload`, `_dict`, `_clean`처럼 이름과 동작이 단순한 private 변환 함수마다 같은 설명을 반복하지 않아 코드가 주석으로 가려지지 않게 했다.
...
- 한글 설명 주석: Python 68/68, 총 933줄
- Python strict UTF-8 decode: 68/68
- UTF-8 BOM: 0건
- 깨짐 대체문자·NUL: 0건
- Python AST/compile: 68/68
- 컴포넌트 Markdown strict UTF-8/BOM 검사: 26/26
- 내장 Prompt 원본과 JSON 3계층 exact match: 9/9
- 현재 JSON strict UTF-8/parse: 16/16
- JSON 내장 Custom Component 코드 검사: 225건
- Python 원본과 JSON source sync: export·개별 import·통합 bundle 각각 75/75
- 비활성 Python: 0개
- ZIP UTF-8/JSON 검사: 10/10
- pytest: 222/222
- 대표 dummy 질문: 23/23
- Langflow 1.8.2 / LFX 0.3.4 전체 node template: 115/115
