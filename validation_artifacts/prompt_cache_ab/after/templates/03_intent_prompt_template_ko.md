...
- Report 후속 질문의 자연어 조건이 `semantic_filters[].aliases` 한 항목과 유일하게 일치하면 해당 항목의 `source_alias`, `column`, `operator`, `value`를 그대로 `condition_resolution.effective_filters`와 `apply_filters` 단계에 사용한다. 예시 문구나 KPI 표시명을 보고 다른 컬럼·값으로 번역하지 않는다.
